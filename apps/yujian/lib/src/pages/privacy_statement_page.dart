    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          h('一句话'),
          p('本地运行的单机记账：不用注册，账本不上云，没有广告和统计。大模型和云端语音都是可选的，要用得你自己配置，发送前卡号、手机号等先打码；除此之外只有检查更新和你主动发的反馈会联网，每一次都记在「出网记录」里。源码公开，可以逐行核对，放心授权。打开「纯本地模式」，连这些也一键掐掉。'),
          ...
          row('同步与云备份（你自己填了服务器）', '同步：账本变更，默认明文（走 HTTPS）；设了「同步加密口令」就只发密文，服务器看不到金额、商户和账户名。备份：整本账本用你的口令 AES-GCM 加密后的密文，没口令谁也解不开', '你自己填的那台服务器（余见不提供）', '每次打开 App 自动同步一轮；备份要你手动点', '不填服务器地址'),
          row('版本检查', '只有请求本身：服务器能看到你的 IP 和 App 版本号，不带任何账本数据', 'yujian.ivyea.com（余见唯一的自家域名：版本检查、离线模型包、安装包备用线路、接收你主动发的反馈）', '每天最多一次；纯本地模式下只有你手动点「检查更新」', '开「纯本地模式」'),
          row('下载离线语音包 / 新版本安装包', '只下载，不上传', '安装包默认从 GitHub（github.com）下载，下不动时换 yujian.ivyea.com 备用线路；离线语音包从 yujian.ivyea.com', '你手动点下载时', '不点'),
          ...
          h('哪些永远不出手机'),
          li('账本本身：yujian.db 数据库文件、账户、分类、每一笔记录、草稿、审计日志', sub: '也不进手机系统的自动备份（Google 云备份、换机迁移都排除了）；换机请用「数据」页导出 / 备份'),
          ...
          li('余见的服务器 yujian.ivyea.com 只做三件事：放版本号文件和离线模型包、当安装包的备用下载线路、接收你主动发送的反馈。不接收账本，不做任何统计'),
